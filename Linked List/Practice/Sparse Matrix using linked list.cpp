...
{
	int row,column,val;
	for(int i=0;i<m;i++)/*first we have to initialize the each element of the array 
	to NULL*/
	{
		//A[i]=new node;
		A[i]=NULL;
...
void display(int m,int n,struct node *p[])
{
	for(int i=0;i<m;i++)
	{
		...
		for(int j=0;j<n;j++)
		{
			if(t and j==t->col)//display if the element is present in linked list 
			{
				cout<<t->value<<" ";
				t=t->next;
			}
