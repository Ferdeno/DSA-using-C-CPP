...
pr->height=nodeheight(pr); //changes 
...
else if(balancefactor(p)==-2 && balancefactor(p->rchild)==1) //changes
...
// cout<<balancefactor(root)<<balancefactor(root->rchild)<<" "<<root->data<<endl;
preorder(root);
